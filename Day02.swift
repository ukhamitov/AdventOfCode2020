...
//  Day02.swift
//
//  Created by Ural Khamitov on 02.12.2020.
//
...
func parseLine1(_ line: String) -> Bool {
    let parts = line.split(separator: ":")
    
    let firstPart = parts[0].split { $0.isWhitespace }
    let secondPart = parts[1]
    
    let rangeStr = firstPart[0]
    let min = Int(rangeStr.split(separator: "-")[0])!
    let max = Int(rangeStr.split(separator: "-")[1])!
    
    let letter = firstPart[1]
    
    let tok = secondPart.components(separatedBy:letter)
    let cnt = tok.count - 1
    if cnt >= min && cnt <= max {
        return true
    } else {
...
let arr = inputStr.split { $0.isNewline }.map { String($0) }
var count1 = 0
for line in arr {
    if parseLine1(line) {
        count1 += 1
...
print("Answer for first part of puzzle: \(count1)")

func parseLine2(_ line: String) -> Bool {
    let parts = line.split(separator: ":")
    ...
    let rangeStr = firstPart[0]
    let firstPos = Int(rangeStr.split(separator: "-")[0])!
    let secondPos = Int(rangeStr.split(separator: "-")[1])!
    
    let letter = String(firstPart[1])
    
    let x = String(secondPart[secondPart.index(secondPart.startIndex, offsetBy: firstPos)])
    let y = String(secondPart[secondPart.index(secondPart.startIndex, offsetBy: secondPos)])
...
print("Answer for second part of puzzle: \(count2)")
